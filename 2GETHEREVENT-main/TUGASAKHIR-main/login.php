...
if (isset($_POST['login'])) {
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        echo "<div id='popup' class='bg-red-100 text-red-800 p-4 rounded-md'>Login gagal. Email atau password salah.</div>";
    }
}
...
if (isset($_POST['register'])) {
    ...
    if ($password !== $confirm) {
        echo "<div id='popup' class='bg-yellow-100 text-yellow-800 p-4 rounded-md'>Password tidak cocok.</div>";
    } else {
        $stmt = $pdo->prepare("SELECT * FROM users WHERE email = ?");
        $stmt->execute([$email]);
        if ($stmt->rowCount() > 0) {
            echo "<div id='popup' class='bg-yellow-100 text-yellow-800 p-4 rounded-md'>Email sudah digunakan.</div>";
        } else {
            $hash = password_hash($password, PASSWORD_DEFAULT);
            $stmt = $pdo->prepare("INSERT INTO users (email, password) VALUES (?, ?)");
            $stmt->execute([$email, $hash]);
            echo "<div id='popup' class='bg-green-100 text-green-800 p-4 rounded-md'>Registrasi berhasil! Silakan login.</div>";
        }
    }
}
?>
